...
int parseImage(char* image, BOOL writeToDisk, NSString* outputDir, BOOL getSymbols,
  ...
  for (unsigned i = 0; i < count; i++) {
    ...
    if (shouldImportStructs && writeToDisk) {
      int firstImport = [dumpString rangeOfString:@"#import"].location != NSNotFound
    ...
    if (isSharedCacheRecursive) {
      ...
      if (fp == NULL) {
        if ([[NSFileManager defaultManager]
                fileExistsAtPath:@"/private/var/db/dyld/dyld_shared_cache_x86_64"]) {
          filename = "/private/var/db/dyld/dyld_shared_cache_x86_64";
        } else if ([[NSFileManager defaultManager]
                       fileExistsAtPath:@"/private/var/db/dyld/dyld_shared_cache_i386"]) {
